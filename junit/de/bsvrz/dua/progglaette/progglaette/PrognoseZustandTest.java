...
public class PrognoseZustandTest extends PrognoseZustand {
	...
	/**
	 * Extrapoliert aus der Tabelle der x,y werte mit Hilfe der kleinsten
	 * Kvadrate ( Least square method) eine lineare funktion und extrapoliert
	 * dem Wert im Zeitpunkt t.
	 * 
	 * @param werte
	 *            Y-Werte
	 * @param zeitPunkte
	 *            X-Werte
	 * @param t
	 *            X-Wert zu dem man Y extrapolieren moechtet
	 * @return Y-Wert zum t
	 */
	public double trendExtrapolationKorrekt(double[] werte, long[] zeitPunkte,
			double t) {
	...
	public double trendExtrapolation(double[] werte, long[] zeitPunkte, double t) {
		...
		for (int i = 0; i < zeitPunkte.length; i++) {
			summeKvadrat += zeitPunkte[i] * zeitPunkte[i] - n * zeitMittel
					* zeitMittel;
		}
	 ...
	 * @return Array von zufaelligen Werten
	 */
	public double[] generiereRandomDoubleArray(double min, double max,
			int anzahl) {
	...
	public void testTrendExtrapolation() {
		...
		int[] trendC = new int[] { 5, 15, 30, 60, 90 };
		...
		for (int i = 0; i < trend.length; i++) {
			trend[i] = trendC[i] * MIN_IN_MS + x[x.length - 1];
		}
		...
		for (int i = 0; i < c.length; i++) {
			...
			System.out.println(String.format(
					"[ %4d ] Prognose: %10.8f == %10.8f  Differrez: %10.8f", i,
					b[i], c[i], b[i] - c[i]));
		}
		...
		for (int k = 0; k < 1000; k++) {
			...
			for (int i = 0; i < c.length; i++) {
				...
				System.out
						.println(String
								.format(
										"[ %4d ] Prognose: %10.8f == %10.8f  Differrez: %10.8f",
										i, b[i], c[i], b[i] - c[i]));
			}
		}
	}
